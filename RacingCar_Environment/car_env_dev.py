...
checkpoint_dir = os.path.join('Training', 'Saved Models', 'checkpoints')
checkpoint_callback = CheckpointCallback(
    save_freq=save_freq,
    save_path=checkpoint_dir,
    name_prefix='Car_model_1'
)

eval_callback = EvalCallback(
    env,
    best_model_save_path=os.path.join('Training', 'Saved Models', 'best_model'),
    log_path=os.path.join('Training', 'Logs', 'eval_results'),
    eval_freq=100000,  # Evaluate every 100,000 steps
    deterministic=True,
    render=False
)
...
PPO_Path = os.path.join('Training','Saved Models','Model_RacingCar')
