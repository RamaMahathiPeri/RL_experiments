...
log_path = os.path.join('Training', 'Logs_curriculum_threshold_1')


for stage,hole_percentage in enumerate(curriculum,1):
    if stage == 1:
        print('we are in stage 1: starting a new model')
        PPO_Path= os.path.join('training_flt_4',f'saved_models_stage_{stage}',f'frozenlake_stage_{stage}.zip')
        ...
        PPO_Path= os.path.join('training_flt_4',f'saved_models_stage_{stage-1}',f'frozenlake_stage_{stage-1}.zip')
        print('######')
        print(PPO_Path)
        print('########')
    ...
    # Save the model
    PPO_Path = os.path.join('training_flt_4', f'Saved_Models_Stage_{stage}', f'FrozenLake_Stage_{stage}')
