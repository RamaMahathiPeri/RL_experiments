...
def testEnv(env):
        for a in range(10):
            ...
            while not (terminated[0] or truncated[0]):
                ...
                print("action size ",env.action_space.sample().size)
                print("action",env.step(action))
                observation, reward, terminated, info = env.step(action)
                print(f"Observation: {observation}, Reward: {reward}, Done: {terminated}, Info: {info}")
                print('########################')
